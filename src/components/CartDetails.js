  ...
  return (
    <div className={`flex justify-center items-center mt-8 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
      <div className="w-full max-w-3xl px-4 sm:px-6 lg:px-8">
        <div className={`bg-${isDarkMode ? 'gray-800' : 'white'} shadow-xl rounded-lg overflow-hidden`}>
          <div className="px-6 py-4">
            <h2 className={`text-3xl font-semibold mb-6 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
              Shopping Cart
            </h2>
            {cart.length === 0 ? (
              <div className="text-center py-8">
                <p className={`text-xl ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Your cart is empty</p>
                <Link
                  to="/"
                  className={`mt-4 text-indigo-600 hover:text-indigo-500 font-medium transition-all duration-200`}
                >
                  Continue Shopping
                </Link>
              </div>
              ...
              cart.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center mb-6 p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} hover:bg-gray-100 transition-all duration-200`}
                >
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  <div className="flex-1">
                    ...
                    <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>${item.price}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Size: {item.size}</p>
                    <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Color: {item.color}</p>
                  </div>
                  <button
                    onClick={() => handleRemoveFromCart(item.id)}
                    className={`ml-auto px-3 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-all duration-200`}
                  >
                    Remove
                  </button>
                </div>
